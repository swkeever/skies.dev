...
We'll put our policy in a separate `deployment.json.tpl` file to decouple our
policies from the Terraform scripts.

```json
// terraform/policies/deployment.json.tpl
...
Now, let's create a `iam.tf` and load in this policy.
