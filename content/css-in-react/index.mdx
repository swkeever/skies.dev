...
I thought a lot (approximately _one whole week_ 🙂) about the various ways we
could go about writing CSS in our React projects.
...
<aside><p>
In programming, we should be conscious <em>any time</em> we declare anything globally.
...
let</code> or <code>const</code> as opposed to <code>var</code>—these keywords
...
With React, we colocate our JavaScript and HTML together in the same file, why
shouldn't we also colocate our CSS?
...
writing styles using a library like this. It's something we should consider as
our application grows.

At this point, we've looked at lot of great options. However, all of the options
...
case? We'll have to inject our own styles alongside the frameworks. What's more,
we'd need to make sure the styles we add our consistent with the design system
provided by the CSS framework.
...
We can then use the generated classes as we normally would.
...
([Steve Schoger](https://twitter.com/steveschoger)) create the design system
that we can use?
...
Many classes on a single line admittedly can become hard to read. One solution
inspired by Tailwind Labs developer
...
Tailwind CSS is a great complement to React. It's probably going to be the tool
I reach for to handle my CSS needs in future projects.
