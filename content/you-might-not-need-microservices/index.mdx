...
[Microservices](https://microservices.io/) are a software architectural style
where the application is decomposed into many smaller applications that each
focus on a single business domain. These services usually have their own
database, their own server running the service, their own metrics, etc. This
...
Microservices are a popular architecture for applications to scale for huge
numbers of users. Companies like Amazon, Netflix, and
...
building an MVP, then you may want to develop your application as a monolith
instead. By nature, microservices are distributed systems, and distributed
systems can be extremely complex to reason about, configure, and manage. When
adopting a microservice architecture, you will have to think about the
following.

- To interact with different features of your business, you'll need to set up a
  [discovery mechanism](https://microservices.io/patterns/server-side-discovery.html)
  so your services can interact using interprocess communication (IPC) protocols
  such as REST or gRPC.
- You need to figure out a story for managing transactions across multiple
  services that each have their own database. You can use the
  ...
  services.
- For each service, you'll need to configure and manage infrastructure such as a
  local database, compute, metrics, etc. The devops overhead will increase as a
  result.
...
architecture. In contrast with microservices, you would need to think about the
following with a monolithic architecture.

- Interacting with different features can mostly happen through method calls.
- Handling transactions is simpler because you can use a single ACID-compliant
  database (or whatever database suits your application requirements).
- You only need to provision infrastructure for a single monolith application.
...
seriously consider converting your monolith into microservices. This doesn't
mean you need to completely rewrite your application from scratch to adopt a
...
[Stranger application](https://docs.microsoft.com/en-us/azure/architecture/patterns/strangler)
